...
namespace Pensoft\ImageResize;

use Pensoft\ImageResize\Classes\Resizer;
use Pensoft\ImageResize\Commands\ImageResizeClear;
use Pensoft\ImageResize\Commands\ImageResizeGc;
use Pensoft\ImageResize\Commands\ImageResizeResetPermalinks;
use Pensoft\ImageResize\Models\ImagePermalink;
use Pensoft\ImageResize\Models\Settings;
use Pensoft\ImageResize\ReportWidgets\ImageResizeClearWidget;
use App;
use Artisan;
use DB;
use Event;
...
class Plugin extends PluginBase
{
    ...
    {
        return [
            'name' => 'pensoft.imageresize::lang.plugin.name',
            'description' => 'pensoft.imageresize::lang.plugin.description',
        ...
        return [
            'filters' => [
                'resize' => function ($image, $width, $height = null, $options = []) {
                    ...
                    $resizer->setImage((string) $image);
                    if(substr($resizer->outputExtension(),0,3) === 'svg'){
                        return $image;
                    }
    ...
    public function registerSettings()
    {
        return [
            'settings' => [
                'label'       => 'pensoft.imageresize::lang.plugin.name',
                'description' => 'Manage default settings for the Image Resizer plugin',
                'category'    => 'Content',
                'icon'        => 'icon-image',
                'class'       => 'Pensoft\ImageResize\Models\Settings',
                'permissions' => ['pensoft.imageresize.access_settings'],
                'order'       => 500,
    ...
    {
        return [
            'pensoft.imageresize.access_settings' => ['tab' => 'pensoft.imageresize::lang.permissions.tab', 'label' => 'pensoft.imageresize::lang.permissions.access_settings'],
        ];
    }
    ...
    public function boot()
    {
        Event::listen('backend.page.beforeDisplay', function ($controller, $action, $params) {
            if ($controller instanceof \System\Controllers\Settings) {
                ...
                if ($params === ['pensoft', 'imageresize', 'settings']) {
                    // Add CSS (minor patch)
                    $controller->addCss('/plugins/pensoft/imageresize/assets/settings-patch.css');
                }
            }
        });
